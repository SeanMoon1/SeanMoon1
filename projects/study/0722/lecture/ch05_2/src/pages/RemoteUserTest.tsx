...
export default function RemoteUserTest() {
  ...
  const getRemoteUser = useCallback(() => {
    toggleLoading()
    D.fetchRandomUser().then(user => dispatch(R.setUser(user))).catch(setError).finally(toggleLoading)
  }, [dispatch, toggleLoading])

  const changeName = useCallback(() => {
    toggleLoading()
    D.fetchRandomUser().then(user => dispatch(R.changeName(user.name))).catch(setError).finally(toggleLoading)
  }, [dispatch, toggleLoading])
  ...
  return (
    <section className="mt-4">
        ...
        <div className="flex justify-center p-4">
            ...
            <Button className="btn-sm btn-primary" onClick={changeName}>
                CHANGE NAME
            </Button>
            <Button className="btn-sm btn-primary" onClick={changeEmail}>
                CHANGE EMAIL
            </Button>
            <Button className="btn-sm btn-primary" onClick={changePicture}>
                CHANGE PICTURE
            </Button>
        </div>
        {loading && (
        ...
        <div className="flex justify-center p-4 mt-4">
            ...
            <div className="ml-4">
                <p className="text-xl text-bold">{user.name.title}. {user.name.first} {user.name.last}</p>
